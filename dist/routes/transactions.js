 ...
 */
router.get('/get/:savingId', authenticateToken, (request, response) => {
    /**
     * GET /savings/get
     * AUTH: Bearer <token>
     */
    ...
    database_1.default.query(`SELECT saving.*, saving_goal.name as goal_name FROM saving LEFT JOIN saving_goal ON saving_goal.id = saving.goal_id WHERE saving_goal.id = '${savingId}'`, (error, result) => {
        if (error) {
            console.log(error);
            response
                .status(500)
